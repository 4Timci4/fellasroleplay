...
$applications = getApplications('approved', $limit, $offset);
$total_applications = countApplications('approved');
...
<div class="container mx-auto">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-2">
        <h1 class="text-3xl font-bold text-text-light">Onaylanan <span class="text-primary">Başvurular</span></h1>
        <span class="bg-[#002a00] text-green-300 text-xs font-semibold px-3 py-1 rounded-lg border border-green-500">
        ...
        <table class="min-w-full divide-y divide-primary-dark">
            <thead>
                <tr>
                    ...
                    <th class="py-3 px-4 border-b border-primary-dark bg-[#0a0a0a] text-left text-xs font-medium text-white uppercase tracking-wider">Onay Tarihi</th>
                    <th class="py-3 px-4 border-b border-primary-dark bg-[#0a0a0a] text-left text-xs font-medium text-white uppercase tracking-wider">Onaylayan</th>
            ...
            <tbody class="divide-y divide-primary-dark">
                ...
                <tr class="hover:bg-[#0a0a0a] transition-colors">
                    ...
                    <td class="py-3 px-4 text-primary">
                        <?php 
                        // Discord kullanıcı adını kullan
                        if (!empty($application['admin_discord_username'])) {
                            echo sanitizeOutput($application['admin_discord_username']);
                        } else if (!empty($application['admin_discord_id'])) {
                            // Discord ID varsa ama kullanıcı adı yoksa, kullanıcı adını getirmeye çalış
                            $discordUsername = getDiscordUsername($application['admin_discord_id']);
                            if (!empty($discordUsername)) {
                                echo sanitizeOutput($discordUsername);
                            } else {
                                echo sanitizeOutput($application['admin_discord_id']);
                            }
                        } else {
                    ...
                    <td class="py-3 px-4">
                        ...
                        <?php if (isAdmin()): ?>
                        <a href="delete_application?id=<?php echo $application['id']; ?>&redirect=approved" 
    ...
    <div class="card-bg rounded-lg shadow-lg p-8 text-center border border-primary-dark">
        ...
        <p class="text-white">Onaylanmış başvuru bulunmuyor.</p>
    </div>
    <?php endif; ?>
</div>
